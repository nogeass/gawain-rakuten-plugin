#!/usr/bin/env node
/**
 * Demo CLI for gawain-rakuten-plugin
...
import { convertRakutenProduct, validateRakutenProduct } from './platform/rakuten_adapter.js';
...
function parseCliArgs(): { productPath: string } {
  ...
  if (values.help) {
    console.info(`
gawain-rakuten-plugin Demo CLI
...
async function main(): Promise<void> {
  console.info('=== Gawain Rakuten Plugin Demo ===\n');
  ...
  if (!validateRakutenProduct(rawProduct)) {
    console.error('Invalid product format. Expected Rakuten product structure.');
    process.exit(1);
  }

  const productInput = convertRakutenProduct(rawProduct);
